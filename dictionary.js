...
function displayloder(){
  ...
  loader.innerHTML = `
  <div id="loader" >
        <center><img style="width: 30%; height:30%" src="load-gif.gif" alt="" />
      </div>
  `;
}
...
function displaymeaning(searchValue) {
  ...
  fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${searchValue}`)
    ...
    .then((data) => {
      ...
      const resultdiv = document.createElement("div");//creating result div to show result dayta
      resultdiv.innerHTML = `
        <h2 class="word">${word}</h2>
        `;
      for (let i = 0; i < data[0].meanings.length; i++) {//each meaning has one data array
